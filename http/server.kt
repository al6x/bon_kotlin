...
open class Server (
  ...
  val is_privacy_enabled:     Boolean = false,
  ...
  fun on(method: String, route: String, handler: Handler): Void {
    ...
    when (method) {
      "get"  -> http.get(route, to_spark_handler(handler))
      "post" -> http.post(route, to_spark_handler(handler))
  ...
  private fun to_spark_handler(handler: Handler): (spark.Request, spark.Response) -> String  {
    return lambda@ { sreq, sres ->
      ...
      val req = try {
        ...
        try {
          val e = if (is_privacy_enabled) Exception("Error [hidden]") else unsafe_error
      ...
      // Handling request
      try {
        log.info(if(is_privacy_enabled) "[hidden]" else req.to_string())
        ...
        try {
          val e        = if (is_privacy_enabled) Exception("Error [hidden]") else unsafe_error
          val req_info = if(is_privacy_enabled) "[hidden]"                   else req.to_string()
